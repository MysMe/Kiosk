...
bool process::isInPosition(rect area) const
{
    ...
    if (shiftPress) XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Shift_L), True, 0);
    if (controlPress) XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Control_L), True, 0);
    if (altPress) XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Alt_L), True, 0);
    XTestFakeKeyEvent(display, keycode, True, 0);
    XTestFakeKeyEvent(display, keycode, False, 0);
    if (shiftPress) XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Shift_L), False, 0);
    if (controlPress) XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Control_L), False, 0);
    if (altPress) XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Alt_L), False, 0);
    XFlush(display);
    XCloseDisplay(display);
    XSetErrorHandler(oldHandler);
    }
    ...
    rect process::getBounds() const {
        ...
        auto oldHandler = XSetErrorHandler([](Display*, XErrorEvent*) { return 0; });
        Display* display = XOpenDisplay(nullptr);
        if (!display || wHandle == 0) return rect{0,0,0,0};
        XWindowAttributes attr;
